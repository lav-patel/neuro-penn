...
readmissions <- clin_raw %>% 
  group_by(patient_num) %>% 
  mutate(delta_hospitalized = diff(c(calendar_date[1], calendar_date))) %>% 
  ungroup() %>% 
...
demo_df <- demo_raw %>%
  ...
  mutate(neuro_post = fct_relevel(neuro_type, neuro_types)) %>% 
...
elix_obfus_table1 <- 
  Reduce(function(...) left_join(..., by = c("Comorbidity", "Abbreviation")), 
       lapply(neuro_types, list_table1, 
              df = scores_unique,
...
elix_obfus_table1 %>% 
  transmute(
    ...
    None = concat(n_None, prop_None),
    Peripheral = concat(n_Peripheral, prop_Peripheral),
    Central = concat(n_Central, prop_Central),
    Both = concat(n_Both, prop_Both),
